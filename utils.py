...
def _display_detected_frames(conf, model_object, model_char, st_count, st_frame, image):
    ...
    for i in res_object:
        ...
        for box in bbox:
            ...
            if cls_names == 1:
                cv2.putText(res_plotted_umat, f'{confs}', (max(40, x2 + 5), max(40, y2 + 5)), fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale=0.5, color=(0, 20, 255),thickness=1, lineType=cv2.LINE_AA)
            elif cls_names == 0:
                cv2.putText(res_plotted_umat, f'{confs}', (max(40, x1), max(40, y1)), fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale=0.6, color=(0, 20, 255),thickness=1, lineType=cv2.LINE_AA)
            ...
            if cls_names == 1:
                ...
                try:
                    ...
                    if len(char_display) == 8:
                        ...
                        cv2.putText(res_plotted_umat, char_result , (max(40, x1 - 15), max(40, y1 - 5)), fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale=0.5, color=(10, 50, 255),thickness=1, lineType=cv2.LINE_AA)
